    ...
    function rtcl_delete_listing() {
        ...
        if (Functions::verify_nonce()) {
            ...
            if ($post_id && Functions::current_user_can('delete_' . rtcl()->post_type, $post_id)) {
                ...
                //update free ads back to 2
                $free_ads = get_user_meta(get_current_user_id(),'_rtcl_ads', true);
                if( $free_ads === '1'){
                    update_user_meta( get_current_user_id(), '_rtcl_ads', '2' );
                }
        ...
        if (Functions::verify_nonce()) {
            ...
            if (Functions::is_enable_terms_conditions() && !$agree) {
                ...
                if (!$cat_id && !$post_id) {
                    ...
                    if (isset($_POST['item']) && isset($_POST['size']) && isset($_POST['item_quantity']) && isset($_POST['condition']) ) {
                          $item = $_POST['item'];    
                          $size = $_POST['size'];
                          ...
                          foreach ($item as $key => $value) {
                                $item_details[] = array(
                                    "item" => Functions::sanitize($value),
                                    "size" => Functions::sanitize($size[$key]),
                                    "qty" => Functions::sanitize($qty[$key]),
                                    "condition" => Functions::sanitize($condition[$key])
                                );
                               
                          }
                        $meta['items'] = serialize($item_details);
                    }
